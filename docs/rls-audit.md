...
As of April 19, 2026 (PR #44, lint guardrail), **44 files** reference `@/db` (system role, bypasses all RLS) — 16 more than the original 28 counted in this audit, reflecting drift. Breakdown:

- **5** are docs/plan Markdown files in `docs/` — reference-only, not linted.
- **11** are legitimate system callers (crons, background AI jobs, analytics, digest, figma-sync, onboarding bootstrap, published-view public route) and are allowlisted in `.eslintrc.json`.
- **28** are user-facing `.ts`/`.tsx` files (settings pages, dashboard pages, server actions, nav queries, `/api/nav`, and similar) that should migrate to `withUserDb` / `withUserSession`. Each shows up as an ESLint warning after this PR.

A `no-restricted-imports` ESLint rule now warns on any new `@/db` import outside the allowlist. The rule is intentionally at `warn` level (not `error`) so CI stays green while the 28-file migration backlog is worked down. Migration and rule-upgrade (warn → error) tracked in **Issue #42**. Automated inventory script tracked in **Issue #43**.
...
| Category | Count |
|---|---|
| System (allowlisted) | 12 |
| System (needs migration) | 30 |
| User-scoped (`@/db/user`) | 52 |
| **Total callers** | **94** |

### System callers — needs migration to `withUserDb` / `withUserSession`

| # | File |
|---|---|
| 1 | `app/(dashboard)/dashboard/ideas/page.tsx` |
| 2 | `app/(dashboard)/dashboard/inbox/page.tsx` |
| 3 | `app/(dashboard)/dashboard/initiatives/[id]/page.tsx` |
| 4 | `app/(dashboard)/dashboard/initiatives/page.tsx` |
| 5 | `app/(dashboard)/dashboard/my-requests/page.tsx` |
| 6 | `app/(dashboard)/dashboard/page.tsx` |
| 7 | `app/(dashboard)/dashboard/radar/page.tsx` |
| 8 | `app/(dashboard)/dashboard/requests/page.tsx` |
| 9 | `app/(dashboard)/dashboard/submitted/page.tsx` |
| 10 | `app/(dashboard)/dashboard/teams/[slug]/active-requests/page.tsx` |
| 11 | `app/(dashboard)/dashboard/teams/[slug]/commitments/page.tsx` |
| 12 | `app/(onboarding)/onboarding/page.tsx` |
| 13 | `app/(settings)/settings/account/page.tsx` |
| 14 | `app/(settings)/settings/danger/page.tsx` |
| 15 | `app/(settings)/settings/integrations/page.tsx` |
| 16 | `app/(settings)/settings/layout.tsx` |
| 17 | `app/(settings)/settings/members/page.tsx` |
| 18 | `app/(settings)/settings/plan/page.tsx` |
| 19 | `app/(settings)/settings/projects/page.tsx` |
| 20 | `app/(settings)/settings/workspace/page.tsx` |
| 21 | `app/actions/activity-log.ts` |
| 22 | `app/actions/clear-sample-team.ts` |
| 23 | `app/actions/get-seen-hints.ts` |
| 24 | `app/actions/mark-hint-seen.ts` |
| 25 | `app/actions/onboarding.ts` |
| 26 | `app/actions/published-views.ts` |
| 27 | `app/actions/seed-sample-team.ts` |
| 28 | `app/api/nav/route.ts` |
| 29 | `lib/bootstrap-access.ts` |
| 30 | `lib/queries/nav.ts` |

### System callers — allowlisted (legitimate system paths)

| # | File |
|---|---|
| 1 | `app/api/cron/alerts/route.ts` |
| 2 | `app/api/cron/morning-briefing/route.ts` |
| 3 | `app/api/cron/resurface/route.ts` |
| 4 | `app/api/cron/weekly-digest/route.ts` |
| 5 | `app/views/[id]/page.tsx` |
| 6 | `lib/ai/morning-briefing.ts` |
| 7 | `lib/alerts/detect.ts` |
| 8 | `lib/analytics/track.ts` |
| 9 | `lib/dashboard/shell-data.ts` |
| 10 | `lib/digest.ts` |
| 11 | `lib/figma/sync.ts` |
| 12 | `lib/onboarding/detect-persona.ts` |

### User-scoped callers

| # | File |
|---|---|
| 1 | `app/actions/cycles.ts` |
| 2 | `app/actions/decision-log.ts` |
| 3 | `app/actions/design-stages.ts` |
| 4 | `app/actions/initiatives.ts` |
| 5 | `app/actions/iterations.ts` |
| 6 | `app/actions/notification-preferences.ts` |
| 7 | `app/actions/projects.ts` |
| 8 | `app/actions/requests.ts` |
| 9 | `app/actions/settings.ts` |
| 10 | `app/actions/stickies.ts` |
| 11 | `app/api/digest/route.ts` |
| 12 | `app/api/figma/oauth/callback/route.ts` |
| 13 | `app/api/figma/oauth/connect/route.ts` |
| 14 | `app/api/figma/oauth/disconnect/route.ts` |
| 15 | `app/api/ideas/[id]/validate/route.ts` |
| 16 | `app/api/ideas/[id]/vote/route.ts` |
| 17 | `app/api/ideas/route.ts` |
| 18 | `app/api/insights/design-roi/route.ts` |
| 19 | `app/api/morning-briefing/route.ts` |
| 20 | `app/api/notifications/route.ts` |
| 21 | `app/api/pm/calibration/route.ts` |
| 22 | `app/api/pm/prior-calibrations/route.ts` |
| 23 | `app/api/requests/[id]/activity/route.ts` |
| 24 | `app/api/requests/[id]/advance-phase/route.ts` |
| 25 | `app/api/requests/[id]/assign-recommend/route.ts` |
| 26 | `app/api/requests/[id]/assign/route.ts` |
| 27 | `app/api/requests/[id]/comment/route.ts` |
| 28 | `app/api/requests/[id]/context-brief/route.ts` |
| 29 | `app/api/requests/[id]/enriched/route.ts` |
| 30 | `app/api/requests/[id]/figma-updates/[updateId]/review/route.ts` |
| 31 | `app/api/requests/[id]/figma-updates/route.ts` |
| 32 | `app/api/requests/[id]/handoff-brief/route.ts` |
| 33 | `app/api/requests/[id]/handoff-checklist/route.ts` |
| 34 | `app/api/requests/[id]/impact-record/route.ts` |
| 35 | `app/api/requests/[id]/impact-retrospective/route.ts` |
| 36 | `app/api/requests/[id]/iteration-summary/route.ts` |
| 37 | `app/api/requests/[id]/kanban/route.ts` |
| 38 | `app/api/requests/[id]/nudge/route.ts` |
| 39 | `app/api/requests/[id]/prediction-confidence/route.ts` |
| 40 | `app/api/requests/[id]/toggle-blocked/route.ts` |
| 41 | `app/api/requests/[id]/triage/route.ts` |
| 42 | `app/api/requests/[id]/update/route.ts` |
| 43 | `app/api/requests/[id]/validate/route.ts` |
| 44 | `app/api/requests/preflight/route.ts` |
| 45 | `app/api/requests/route.ts` |
| 46 | `app/api/team/[memberId]/manager/route.ts` |
| 47 | `app/api/views/[id]/pin/route.ts` |
| 48 | `app/api/views/route.ts` |
| 49 | `lib/auth-context.ts` |
| 50 | `lib/notifications.ts` |
| 51 | `test/db-user.test.ts` |
| 52 | `test/user-session-url.test.ts` |

<!-- db-access-inventory:end -->
