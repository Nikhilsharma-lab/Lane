...
- [2026-04-19] lane dev Supabase project was empty (zero tables in public schema) during pre-A1 work. Discovered at A1 Step 0.3 when SELECT count(*) FROM requests returned "relation does not exist". This means: (a) local Next.js dev has been effectively non-functional at the DB layer, hidden by spec/UI work that didn't exercise DB paths; (b) the "dev/staging split done" entry on ROADMAP line 175 was technically true but functionally hollow — dev project existed but was never seeded. Fix: run drizzle-kit push against DIRECT_DATABASE_URL before Phase B Step B1 to populate lane dev with the full Lane schema. Verify no drift against lane app. Budget: 30-60 min. Does not block A1-A3 (pg-tap harness + Playwright + fixtures don't need the Lane schema).
