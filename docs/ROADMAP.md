...
> **Next session:** Item 4 backend is done (sub-tasks A, C, F). Sub-tasks D (three-panel UI in new-request-form.tsx) and E (POST route classifier fields + server-side enforcement) remain — ~2.5 hours estimated. Read `docs/WORKING-RULES.md` first. Re-read `docs/onboarding-spec.md` section 7 for the exact UI spec. The classifier output is already available from `/api/requests/preflight` response — fields are `classification`, `reframedProblem`, `extractedSolution`. Production issues (ANTHROPIC_API_KEY 401, database pool) still pending; fix before deploying Item 4 to users.
...
- [ ] **Item 4** — Intake check UI build. **In progress (April 16): backend sub-tasks done, UI sub-tasks remain.** Sub-tasks A+F (migration adding 4 classifier columns, spec renamed to match existing intakeJustification), B (reverted — lib/ai/intake-classifier.ts was duplicate of triage's classification), and C (extended triage with extractedSolution field, extended preflight route response) shipped in commits on April 16. Remaining sub-tasks: D (three-panel UI in new-request-form.tsx — problem_framed proceeds silently, solution_specific shows amber panel with three CTAs, hybrid shows extraction panel) and E (update POST /api/requests to write classifier fields + server-side enforcement). Analytics events from onboarding-spec section 10 explicitly deferred per April 16 parking lot decision. Remaining estimate: ~2.5 hours. [STRICT: after V3, after AI foundation verification]
