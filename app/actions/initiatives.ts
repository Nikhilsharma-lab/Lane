...
export async function updateInitiative(
  ...
  return withUserDb(userId, async (db) => {
    ...
    const [initiative] = await db
      .select()
      .from(initiatives)
      .where(and(eq(initiatives.id, initiativeId), eq(initiatives.orgId, profile.orgId)));
...
export async function addRequestToInitiative(initiativeId: string, requestId: string) {
  const userId = await getAuthedUserId();
  if (!userId) return { error: "Not authenticated" };
  ...
  return withUserDb(userId, async (db) => {
    ...
    await db.insert(initiativeRequests).values({ initiativeId, requestId });
...
export async function removeRequestFromInitiative(initiativeId: string, requestId: string) {
  const userId = await getAuthedUserId();
  if (!userId) return { error: "Not authenticated" };
...
export async function getInitiativesForOrg() {
  ...
  return withUserDb(userId, async (db) => {
    ...
    const countMap = Object.fromEntries(countRows.map((r) => [r.initiativeId, Number(r.total)]));
