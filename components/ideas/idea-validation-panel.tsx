    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
      <div className="w-full max-w-lg bg-zinc-950 border border-zinc-800 rounded-2xl overflow-hidden shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-6 py-4 border-b border-zinc-800 flex items-center justify-between sticky top-0 bg-zinc-950">
          <div>
            <h2 className="text-sm font-semibold text-zinc-200">Validate idea</h2>
            <p className="text-xs text-zinc-600 mt-0.5 truncate max-w-xs">{ideaTitle}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-5 space-y-5">
          {loading ? (
            <div className="flex items-center justify-center py-8">
              <div className="w-5 h-5 border border-zinc-600 border-t-transparent rounded-full animate-spin" />
              <span className="text-xs text-zinc-500 ml-3">Running AI analysis...</span>
            </div>
          ) : error && !aiScores ? (
            ...
            <>
              ...
              <div className="border border-zinc-800 rounded-xl p-4 space-y-3">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-[10px] text-zinc-700">· claude-3-5-haiku</span>
                </div>
                ...
                <p className="text-[11px] text-zinc-500 leading-relaxed pt-1">{aiScores.reasoning}</p>
              </div>

              {/* Score overrides */}
              <div>
                <p className="text-xs text-zinc-500 mb-2">Override scores (optional)</p>
                <div className="grid grid-cols-4 gap-2">
                  {(["impactScore", "effortEstimate", "feasibilityScore", "roiScore"] as const).map((key) => {
                    ...
                    return (
                      <div key={key}>
                        <label className="text-[10px] text-zinc-600 block mb-1">{labels[key]}</label>
                        <input
                          type="number"
                          min={1}
                          max={10}
                          ...
                          className="w-full bg-zinc-900 border border-zinc-800 rounded px-2 py-1 text-xs text-zinc-200 placeholder-zinc-700 focus:outline-none focus:border-zinc-600 transition-colors"
                        />
                      </div>
                    );
                  })}
              ...
              {/* Decision */}
              <div>
                <p className="text-xs text-zinc-500 mb-2">Decision</p>
                <div className="flex gap-2">
                  {(["approved", "approved_with_conditions", "rejected"] as const).map((d) => {
                    ...
                    const styles = {
                      approved: decision === d ? "bg-green-500/15 border-green-500/30 text-green-400" : "border-zinc-800 text-zinc-500 hover:text-zinc-300",
                      approved_with_conditions: decision === d ? "bg-amber-500/15 border-amber-500/30 text-amber-400" : "border-zinc-800 text-zinc-500 hover:text-zinc-300",
                      rejected: decision === d ? "bg-red-500/15 border-red-500/30 text-red-400" : "border-zinc-800 text-zinc-500 hover:text-zinc-300",
                    };
                    return (
                      <button
                        key={d}
              ...
              <div className="flex justify-end gap-2 pt-1">
                <button
                  onClick={onClose}
                  className="text-xs text-zinc-500 hover:text-zinc-300 px-4 py-2 transition-colors"
                >
                  Cancel
                </button>
                <button
