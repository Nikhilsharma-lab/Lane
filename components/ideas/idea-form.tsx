    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
      <div className="w-full max-w-lg bg-zinc-950 border border-zinc-800 rounded-2xl overflow-hidden shadow-2xl">
        ...
        <div className="px-6 py-4 border-b border-zinc-800 flex items-center justify-between">
          <h2 className="text-sm font-semibold text-zinc-200">Submit an idea</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          {/* Title */}
          <div>
            <label className="text-xs text-zinc-500 block mb-1">Idea title *</label>
            <input
              type="text"
              value={form.title}
              onChange={(e) => set("title", e.target.value)}
              placeholder="Short, descriptive name"
              className="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2 text-sm text-zinc-200 placeholder-zinc-700 focus:outline-none focus:border-zinc-600 transition-colors"
            />
          </div>

          {/* Problem */}
          <div>
            <label className="text-xs text-zinc-500 block mb-1">What problem does this solve? *</label>
            <textarea
              value={form.problem}
              onChange={(e) => set("problem", e.target.value)}
              rows={3}
              placeholder="Describe the problem clearly..."
              className="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2 text-sm text-zinc-200 placeholder-zinc-700 focus:outline-none focus:border-zinc-600 transition-colors resize-none"
            />
          </div>

          {/* Proposed solution */}
          <div>
            <label className="text-xs text-zinc-500 block mb-1">Proposed solution *</label>
            <textarea
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-xs text-zinc-500 block mb-1">Effort estimate (weeks)</label>
              <input
                type="number"
                min={1}
          ...
          {/* Impact estimate */}
          <div>
            <label className="text-xs text-zinc-500 block mb-1">Expected impact</label>
            <input
              type="text"
          ...
          {/* Target users */}
          <div>
            <label className="text-xs text-zinc-500 block mb-1">Target users</label>
            <input
              type="text"
          ...
          <label className="flex items-center gap-2.5 cursor-pointer">
            ...
            />
            <span className="text-xs text-zinc-400">Submit anonymously</span>
          </label>
          ...
          <div className="flex justify-end gap-2 pt-1">
            ...
            <button
              type="submit"
              disabled={submitting}
              className="text-xs bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2 rounded-lg transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
            >
