...
> **Next session:** B3 — Migration 0014 — `transfer_workspace_ownership` RPC + pg-tap `test/sql/test_migration_0014.sql`. (~2 hours estimated). **Prerequisite: Line 461 pre-B3 blocker** — spec §4.4 / §4.3 still reference `organizations.owner_user_id`; actual SQL column is `owner_id`. Spec must be patched OR SQL column renamed in a prerequisite migration before B3 ships; copying §4.3's `UPDATE organizations SET owner_user_id = ...` verbatim would fail with "column does not exist." See ROADMAP parking lot line 461 for resolution options.
...
- [x] **B2.** Migration 0013 — invite team scoping + unique pending invite index + pg-tap `test/sql/test_migration_0013.sql` (15 assertions, all passing on lane dev). (actual: ~2.5 hours vs ~1.25 hour estimate; overrun absorbed 0012 snapshot-chain repair + dual-naming bug catch pre-append + column-name verification rule codification). Shipped across 3 commits: 2936339 (chore repair) → affabd9 (feat 0013 migration + schema) → 186ca28 (test pg-tap). Applied to lane dev only; lane app apply deferred to separate production session.
...
*Last updated: April 23, 2026 — Week 7.5a Phase A complete (A1, A2a, A2b, A3) + Phase B steps B1 and B2 shipped (migrations 0011 + 0012 + 0013). Next: B3 (migration 0014, transfer_workspace_ownership) — see Line 461 parking-lot entry for pre-B3 blocker.*
...
- [2026-04-20] Spec column-name drift: docs/user-flows-spec.md §4.4 references organizations.owner_user_id; actual schema column is organizations.owner_id (db/schema/users.ts:6). **Trigger reclassified 2026-04-23:** original trigger "when migration 0011 lands in B1" missed at B1 close (cc8e18f patched only §4.1.1/§4.1.2). New trigger: "before B3 composition" — B3's transfer_workspace_ownership spec §4.3 line 435 has `UPDATE organizations SET owner_user_id = ...` which fails verbatim. Options: patch spec (~15 min, 6+ sites) OR rename SQL column in a prerequisite migration (wider blast radius). Decide at B3 pre-work.
...
*From Pre-B1.a/b/c/d:*
- ~~[2026-04-22] §3.3 role-vocabulary confusion — B2's Path C semantic decision.~~ **Resolved 2026-04-23 in commit `affabd9`** — Path C column contract locked to `team_role` only (not `role`); rationale from `project_members.role` usage audit showing text column has zero readers codebase-wide while `team_role` has 3 active read sites (lib/queries/nav.ts, lib/onboarding/detect-persona.ts, lib/nav/order.ts).
...
- ~~[2026-04-22] 0013 audit_log reinstate team_id — B1's accept_invite_membership audit_log INSERT intentionally omits 'team_id', invite_row.team_id because invites.team_id doesn't exist until 0013 (B2, renumbered from 0012 after 0012 was consumed by the RPC ambiguity fix-up). Reinstate the key in B2/0013 when the column is added. One-line edit to the CREATE OR REPLACE FUNCTION body. Budget: 5 min.~~ **Resolved 2026-04-23 in commit `affabd9`** — Pattern (ii) CASE WHEN conditionally includes team_id + team_role keys when invite.team_id IS NOT NULL; workspace-only accepts omit both keys. Queryable with `event_data ? 'team_id'`.
- ~~[2026-04-22] B2 workspace_members.role mapping — B1's accept_invite_membership hardcodes workspace_members.role = 'member' for all invite-accepted signups. Intent-preservation of invites.role (e.g., invitee marked as admin in the invite) is deferred to B2, which touches invite schema anyway. Decide the mapping then. Options: (i) always 'member' permanently, (ii) CASE on invites.role text, (iii) treat invites.role as deprecated in favor of team_role. Budget: 15 min decision + 5 min SQL once decided.~~ **Resolved 2026-04-23 in commit `affabd9`** — option (i) chosen: stays hardcoded 'member'. Rationale: invites don't encode workspace-level authorization; promotion to admin/owner is a separate explicit action (future B3 transfer_workspace_ownership). Comment in RPC body documents the decision.
- ~~[2026-04-22] supabase/test-seed.sql FK violation post-0011 — once migration 0011 applies on lane dev, profiles.id → auth.users(id) CASCADE FK is active. test-seed.sql currently inserts profiles with synthetic UUIDs without seeding auth.users first. Every seed run post-apply fails with FK violation. Fix: add matching INSERT INTO auth.users (id, email) rows (3 lines, id + email only since other cols default) at the top of test-seed.sql before the profiles INSERTs. Trigger: immediately after B1 commits, before next CI run or any test execution. Budget: 10 min including verify.~~ **Resolved 2026-04-22 in commit `78e2f75`** — 4 auth.users rows prepended (minimal columns, ON CONFLICT (id) DO NOTHING for idempotency); seed verified clean on lane dev with correct row counts, idempotent on re-run.
- [2026-04-22] test:sql runner hygiene — `scripts/run-sql-tests.mjs` has three issues surfaced during B1 pg-tap work. (a) Per-file tally parsing (undercounts NOTICE-forwarded rows — dormant post-`ccdf703` since DO-block refactor eliminated NOTICE-based assertions in test_migration_0011, but a future test file that uses NOTICEs would still hit this). (b) NOTICE-forwarded TAP lines emit out of order relative to SELECT rows — same dormant-but-latent status as (a). (c) Transaction-cascade between files: runner uses `{ max: 1 }` connection pooling, so one file's mid-transaction error (aborted via RAISE EXCEPTION before ROLLBACK) poisons subsequent files until they fail with "current transaction is aborted." Symptom observed at STOP F of B1 (pre-0012 fix); structurally present regardless of whether current test files trigger it. Fix: issue `ROLLBACK` between files even on failure, or use a fresh connection per file. Bundle all three. Budget: 45-60 min.
- ~~[2026-04-22] pg-tap DO-block pattern refactor — current test_migration_0011.sql uses `DECLARE tap_line TEXT` + `RAISE NOTICE '%', tap_line` inside DO blocks to make assertion TAP output visible to the runner. Works but architecturally awkward (notices are meant for diagnostics, not pass/fail data). Long-term pattern: refactor each DO block to `CREATE FUNCTION test_N_name() RETURNS SETOF TEXT AS $$ BEGIN ... RETURN NEXT ok(...); END $$;` + `SELECT * FROM test_N_name();`. Emits TAP rows in-order with SELECT output, runner tally counts them natively. Trigger: before B2/B3/B4 pg-tap tests are authored — doing the refactor once on 0011's test establishes the pattern for the rest of Phase B's tests. Budget: 1-2 hours including verify.~~ **Resolved 2026-04-22 in commit `ccdf703`** — 6 DO blocks refactored to `CREATE OR REPLACE FUNCTION test_NN_name() RETURNS SETOF TEXT` + `SELECT * FROM test_NN_name()`. Runner tally now shows `PASS (19 assertion(s))` natively (was 13 under workaround). Rows emit in strict 1..19 order. Pattern documented in CLAUDE.md Part 15 for B2/B3/B4 test files.
...
- [2026-04-22] test-seed.sql invocation path discoverability — the path to run supabase/test-seed.sql is documented only in supabase/README.md (as `psql -f supabase/test-seed.sql`, which requires psql locally — often not installed on macOS). `npm run db:seed` is a different script (demo-org seeding), creating discoverability drift. Reproducible path discovered during pre-B2 cleanup: node + postgres.js + `{ max: 1 }` reservation to handle BEGIN/COMMIT blocks. Fix options: (i) add a `npm run db:seed:test` script entry in package.json wrapping the node+postgres pattern; (ii) add the invocation command to CLAUDE.md Part 15; (iii) both. Lean: (iii). Budget: ~15 min.
- [2026-04-23] Orphan write to project_members.role in app/actions/projects.ts:46 — writes `role: "lead"` to a column with zero readers codebase-wide (surfaced by the 2026-04-23 Path C column-contract audit during B2). Fix: change `role: "lead"` to `teamRole: "lead"` for consistency with seed-sample-team.ts precedent and the 3 active read sites (lib/queries/nav.ts, lib/onboarding/detect-persona.ts, lib/nav/order.ts). 3-line edit. Not B2 blocker — the orphan write persists as-is, B2's Path C uses team_role correctly. Budget: 15 min including verify.
- [2026-04-23] Fix-up migration snapshot discipline — when a fix-up migration ships with no schema changes (e.g., 0012_fix_rpc_org_id_ambiguity), `drizzle-kit generate` is easy to skip since there's nothing to diff. Authoring the snapshot by copying the previous one (byte-identical content, semantically correct) fails to regenerate the `id` and `prevId` metadata, creating a latent chain collision that surfaces at the next real generate. Surfaced B2 PART 2; repaired in commit 2936339. Pattern joins the cluster with §4.4 spec drift (Line 461) and §4.2 spec heading drift (surfaced this session) — all share the shape "a discipline gap ships a latent defect that surfaces at the first downstream operation." Proposed WORKING-RULES rule: fix-up migrations (no schema changes) must run `drizzle-kit generate` anyway, or manually regenerate snapshot id/prevId if the generate doesn't produce a new file; never reuse the previous snapshot's metadata verbatim. Budget: 15 min including rule drafting.
- [2026-04-23] `existing_profile` unused variable in accept_invite_membership RPC — declares `existing_profile public.profiles%ROWTYPE` and runs `SELECT * INTO existing_profile FROM public.profiles WHERE id = target_user_id` just to branch on FOUND afterward. Variable content is never read. Replace with `IF NOT EXISTS (SELECT 1 FROM public.profiles WHERE id = target_user_id) THEN ...` — same semantics, clearer intent. Inherited from B1; B2 preserved the pattern for diff minimization. Not blocker-relevant. Budget: 15 min including pg-tap re-verification.
- [2026-04-23] Spec references to migration filenames drift on renumber — no existing trigger catches this. Example: `user-flows-spec.md` §4.2 heading still says `0012_invite_team_scoping.sql` despite the post-B1 renumber to 0013 (missed in cc8e18f). Joins the Line 461 pattern: triggers fire when someone thinks to look, not when the defect exists. Propose mechanical audit: grep spec for `\d{4}_\w+\.sql` filename references after every migration renumber; cross-check against `db/migrations/` directory listing. Budget: 15 min audit + 5 min any fixes.
- [2026-04-23] Probe script location convention — Lane has three coexisting ad-hoc patterns: `/tmp/` throwaway .ts files (rejected module resolution from outside repo), inline heredocs via `npm run script -- <file>`, and a newly-created `scripts/probes/` subdirectory (B2 session). No formalized convention. Fix: (i) pick canonical location — likely `scripts/probes/` with `.gitignore` entry for the subdirectory; (ii) add `npm run probe` wrapper; (iii) document in CLAUDE.md Part 15 alongside pg-tap testing section. Lean: (i) + (iii). Budget: 30 min.
...
**Active items:** 75 — update whenever parking lot entries are added or resolved. 2026-04-23: 73 → 75 after B2 session — 3 resolutions (Lines 473, 507, 508) + 5 new entries (projects.ts:46 orphan write, fix-up snapshot discipline, existing_profile unused var, spec filename-reference drift, probe-script convention) + 1 reclassification of Line 461 (still active, trigger updated). Previous count of 12 was stale; corrected 2026-04-20 during pre-B1 bootstrap commit.
